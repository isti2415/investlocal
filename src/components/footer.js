      ...
      <Container>
        <div className="grid max-w-screen-xl grid-cols-1 gap-10 pt-10 mx-auto mt-5 border-t border-gray-100 lg:grid-cols-5">
          <div className="lg:col-span-2">
            ...
            <div className="max-w-lg mx-auto mt-4 text-gray-400">
          ...
          <div>
            <div className="grid grid-cols-2 flex-wrap w-full mx-auto mt-8 -ml-3 lg:ml-0">
              {navigation.map((item, index) => (
                <Link key={index} href={item.href}
                  className="w-full px-4 py-2 text-gray-300 hover:text-indigo-500 focus:text-indigo-500 focus:bg-indigo-100 focus:outline-none dark:focus:bg-trueGray-700">
                    {item.name}
                </Link>
              ))}
            </div>
        ...
        <div className="my-10 text-sm text-center text-gray-600 dark:text-gray-400">
          Copyright © {new Date().getFullYear()}. Made with ♥ by Istiaq Ahmed & Faiza Omar Arpita
        </div>
      </Container>
    </div>
  );
